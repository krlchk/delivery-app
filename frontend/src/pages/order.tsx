  ...
  return (
    <main className="flex h-screen flex-col items-center bg-neutral-200 p-10 text-neutral-700">
      ...
      {orderedProducts.length !== 0 ? (
        <OrderSummary orderedProducts={orderedProducts} totalCost={totalCost} />
      ) : (
        <EmptyCartMessage />
      )}
    </main>
...
const OrderSummary = ({ orderedProducts, totalCost }: OrderSummaryProps) => {
  ...
  return (
    <div>
      ...
      <button className="mt-5 w-full rounded-lg bg-green-800 px-4 py-1 font-semibold text-white transition-colors hover:bg-green-900">
        Confirm order
      </button>
      <button
