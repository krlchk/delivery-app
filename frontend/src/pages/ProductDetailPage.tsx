...
import { Footer, Header, InputField, ModalWindow } from "../shared";
...
interface ProductDetailPageMainSectionProps {
  currentProduct: IProduct;
  handleClickDeleteModal: () => void;
}

const ProductDetailPageMainSection = ({
  currentProduct,
  handleClickDeleteModal,
}: ProductDetailPageMainSectionProps) => {
  const dispatch = useAppDispatch();
  ...
  return (
    <main className="flex w-full flex-grow justify-center gap-10 py-14">
      <article
        key={currentProduct?.id}
        className="flex h-1/2 w-1/3 flex-col overflow-hidden rounded-lg border border-neutral-400"
      >
      ...
      <section className="w-1/2">
        ...
        <form className="flex flex-col gap-3" onSubmit={handleUpdateProduct}>
          ...
          <button
            className="w-full rounded-md border bg-green-800 p-1 text-lg font-semibold text-white transition-colors hover:bg-green-900"
            type="submit"
          >
            Update
          </button>
        ...
        <button
          ...
          className="mt-2 w-full rounded-md border bg-red-800 p-1 text-lg font-semibold text-white transition-colors hover:bg-red-900"
          type="submit"
        >
          Delete product
        </button>
